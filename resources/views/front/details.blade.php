...
@section('content')
    ...
    <section id="Category" class="w-full max-w-[1280px] mx-auto px-5 lg:px-[52px] mt-16 mb-[100px]">
        <div class="flex flex-col gap-16">
            ...
            <main class="flex flex-col lg:flex-row gap-8">
                <section id="Details" class="flex flex-col w-full lg:w-[724px] rounded-2xl gap-8 p-8 bg-white">
                        ...
                        <div id="Rating" class="flex items-center gap-4">
                            <div
                                ...
                                <p class="font-medium text-aktiv-green capitalize">
                                    {{ $workshop->category->name ?? 'Category not available' }}</p>
                            </div>
                            ...
                            <div
                                ...
                                <div class="flex flex-col gap-3 justify-between">
                                    <p class="font-medium leading-[25.6px] text-aktiv-grey line-clamp-4">
                                        {{ $workshop->address }}</p>
                                    <a href="https://www.google.com/maps/place/{{ urlencode($workshop->address) }}"
                    ...
                    <div class="flex flex-col rounded-3xl p-8 gap-4 bg-white">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <div class="flex flex-col gap-[2px] flex-1">
                                ...
                                <p class="font-medium text-aktiv-grey">{{ $workshop->instructor->occupation }}r</p>
                            </div>
    ...
    <section class="bg-cover bg-center" style="background-image: url('{{ Storage::url($workshop->venue_thumbnail) }}');">
        <div class="flex items-center justify-center min-h-screen bg-black bg-opacity-50">
            <div class="overflow-hidden flex flex-col md:flex-row w-[1280px] h-[70vh] items-center justify-center">
                <!-- Foto Kiri -->
                <a href="{{ route('front.details', $workshop->slug) }}"
                    class="overflow-hidden flex flex-col items-center justify-center">
                    <img src="{{ Storage::url($workshop->thumbnail) }}" alt="Film Cover" class="w-full h-3/4 rounded-lg">
                    <div class="p-2 text-center">
                        <h2 class="text-lg text-white font-semibold">{{ $workshop->name }}</h2>
                    </div>
                </a>
                <!-- Konten Kanan -->
                <div class="w-full md:w-1/2 p-8 flex flex-col justify-center">
                    <h2 class="text-3xl font-bold text-white mb-4">{{ $workshop->name }}</h2>
                    <p class="text-white text-lg">{{ $workshop->about }}</p>
                    <div class="flex gap-4 my-3">
                        @forelse ($workshop->benefits as $itemBenefit)
                            <div class="bg-yellow-500 p-2 rounded-lg text-white">{{ $itemBenefit->name }}</div>
                        @empty
                            <p>Belum ada kerjasama dengan pihak teater</p>
                        @endforelse
                    </div>
                    <p class="text-white">{{ $workshop->time_at->format('h:i A') }}</p>
                    <a href="{{ route('front.booking', $workshop->slug) }}"
                        class="mt-4 w-full lg:w-40 px-6 py-2 bg-blue-950 text-yellow-700 font-bold rounded-lg">BUY A
                        TICKET</a>
                </div>
            </div>
        </div>
